                    ...
                    using (SqlCommand command = new SqlCommand(sql, conn))
                    {
                        ...
                        if (exists != null) //if its true
                        {
                            ...
                            sql = "UPDATE dbo.staff_overtime SET prior_work_day = " + prior.ToString() + ", post_work_day = " + post.ToString() + " " +
                                "WHERE date = '" + _passedDate.ToString("yyyy-MM-dd") + "' AND staff_id = " + dgOverTime.Rows[i].Cells[staffIndex].Value.ToString();
                        }
                        else //not true
                        {
                            //insert
                            sql = "INSERT INTO dbo.staff_overtime (staff_id,prior_work_day,post_work_day,date) " +
                                "VALUES (" + dgOverTime.Rows[i].Cells[staffIndex].Value.ToString() + "," + prior + "," + post + ",'" + _passedDate.ToString("yyyy-MM-dd") + "')";
                        }
                        using (SqlCommand cmd = new SqlCommand(sql, conn))
                            cmd.ExecuteNonQuery();
                    }
